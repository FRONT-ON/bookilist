...
import {
	...
	BestsellerListDTO,
	BookDetails,
	GoogleBookSearchResultDTO,
	ListNameDTO,
} from '@backend/models/bestsellers';
...
const getBestsellerListData = async (randomListNames: string[]) => {
	const randomListsDataResponse = await axios.all(
		randomListNames.map((randomListName) => {
			return axios.get<BestsellerListDTO>('https://api.nytimes.com/svc/books/v3/lists.json', {
	...
	const randomBooksFromLists = responsesData.reduce((accumulator: BookDetails[], currentValue: Bestseller[]) => {
		...
		const randomBooksDetails = randomBookIndexes.map(
			(randomBookIndex) => currentValue[randomBookIndex].book_details[0],
		);
...
const getGoogleBooksImages = async (bestsellerListData: BookDetails[]) => {
	const googleBooksDataResponse = await axios.all(
		bestsellerListData.map((randomBooksFromList) => {
			return axios.get<GoogleBookSearchResultDTO>('https://www.googleapis.com/books/v1/volumes', {
				params: {
					key: process.env.GOOGLE_API_KEY,
					q: `${randomBooksFromList.title}+isbn:${randomBooksFromList.primary_isbn13}`,
				},
			});
		}),
	);

	const googleBooksData = googleBooksDataResponse.map(
		(response) => response.data.items[0].volumeInfo.imageLinks.thumbnail,
	);
